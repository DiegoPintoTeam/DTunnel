...
class InputBoolean extends Input {
    __html = `
        <button type="button" class="btn btn-status-toggle"></button>`
    ...
    constructor(config) {
        ...
        this.button = this.element.querySelector('button');
        this.value = Boolean(config.value);
    ...
    updateButton() {
        this.button.classList.toggle('is-active', this.value);
        this.button.classList.toggle('is-inactive', !this.value);
        this.button.textContent = this.value ? 'SI' : 'NO';
        this.button.setAttribute('aria-pressed', String(this.value));
    }
