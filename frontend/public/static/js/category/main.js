...
const main = async () => {
    ...
    const render = () => {
        ...
        const items = categories.map(c => {
            ...
            item.setOnClickDelete(() => {
                showAlertConfirm(() => categoryList.remove(c), 'Esta acción eliminará la categoría y sus configuraciones asociadas.', '¿Estás seguro de que deseas borrar?')
            })
        ...
        currentTable = new CategoryTable(items)
        root.appendChild(currentTable.render())
    }
    ...
    categoryList.register(new Observer('add', async category => {
        ...
        if (response.status == 201) {
            ...
            showToastSuccess(`Categoría ${category.name} creada con éxito!`)
            render()
            return
        }
    ...
    categoryList.register(new Observer('remove', async category => {
        ...
        try {
            ...
            if (response.status == 204) {
                showToastSuccess(`Categoría ${category.name} eliminada con éxito!`)
                return
            }
            ...
            showToastError(`Error al borrar categoría ${category.name}!`)
        }
    ...
    categoryList.register(new Observer('update', async category => {
        ...
        if (data.status == 200) {
            showToastSuccess(`Categoría ${category.name} actualizada con éxito!`)
            return;
        }
    ...
    btnDelete.setOnClick(() => {
        if (!currentTable) {
            showToastError('Seleccione al menos una categoría.');
            return;
        }

        const checkedItems = currentTable.getCheckedItems();
        ...
        }, 'Esta acción eliminará las categorías seleccionadas y sus configuraciones asociadas.', '¿Estás seguro de que deseas borrar?');
    });
