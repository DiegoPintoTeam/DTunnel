import ConfigModel, { Mode, ModeLabels } from "../models.js"
import {
    ConfigCategory,
    ConfigDesc,
...
class ConfigForm {
    constructor(config, categories) {
        ...
        this.mode = new ConfigType(Object.keys(Mode).map(key => ({
            value: Mode[key],
            label: ModeLabels[key] || Mode[key]
        })))
...
class ConfigFormSshDirect extends ConfigForm {
    constructor(config, categories) {
        ...
        this.server = new ConfigServer(config?.server?.host || '', 'IP o dominio del servidor')
        this.serverPort = new ConfigPort(config?.server?.port || 80, 'Puerto Servidor SSH', '80')
...
class ConfigFormSshProxy extends ConfigFormSshDirect {
    constructor(config, categories) {
        super(config, categories)
        this.serverPort = new ConfigPort(config?.server?.port || 22, 'Puerto Servidor SSH', '22')
        this.proxy = new ConfigProxy(config?.proxy?.host || '', 'IP o dominio del proxy')
        this.proxyPort = new ConfigPort(config?.proxy?.port || 80, 'Puerto Proxy (HTTP)', '80')
    }
...
class ConfigFormSslDirect extends ConfigForm {
    constructor(config, categories) {
        ...
        this.server = new ConfigServer(config?.server?.host || '', 'IP o dominio del servidor')
        this.serverPort = new ConfigPort(config?.server?.port || 443, 'Puerto SSL / TLS (Servidor)', '443')
        this.dns1 = new ConfigDns1(config?.dns_server?.dns1 || '8.8.8.8')
        this.dns2 = new ConfigDns2(config?.dns_server?.dns2 || '8.8.4.4')
        this.username = new ConfigUsername(config?.auth?.username || '')
        this.password = new ConfigPassword(config?.auth?.password || '')
...
class ConfigFormSslProxy extends ConfigFormSslDirect {
    constructor(config, categories) {
        super(config, categories)
        this.serverPort = new ConfigPort(config?.server?.port || 443, 'Puerto Servidor (SSL)', '443')
        this.payload = new ConfigPayload(config?.config_payload?.payload || '')
        this.proxy = new ConfigProxy(config?.proxy?.host || '', 'IP o dominio del proxy')
        this.proxyPort = new ConfigPort(config?.proxy?.port || 80, 'Puerto Proxy Remoto', '80')
    }
...
class ConfigFormOpenVPNSsl extends ConfigFormOpenVPN {
    constructor(config, categories) {
        super(config, categories)
        this.sni = new ConfigSni(config?.config_payload?.sni || '')
        this.proxyPort = new ConfigPort(config?.proxy?.port || 443, 'Puerto SSL (SNI)', '443')
        this.tlsVersion = new ConfigTlsVersion(['TLSv1.3', 'TLSv1.2', 'TLSv1.1'])
        this.tlsVersion.setSelected(config.tls_version)
    }
