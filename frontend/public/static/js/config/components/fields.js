...
export class ConfigType extends SelectFiled {
    constructor(types) {
        const options = types.map(v => (typeof v === 'object' && v.label ? v : { label: v, value: v }))
        super(options)
...
export class ConfigProxy extends TextFiled {
    constructor(proxy, placeholder = '127.0.0.1 o dominio') {
        super()
        this._element.value = proxy
        if (placeholder) this._element.placeholder = placeholder
        this._label.innerHTML = 'Proxy Remoto'
    }
...
export class ConfigServer extends TextFiled {
    constructor(server, placeholder = 'IP del servidor o dominio') {
        super()
...
export class ConfigPort extends NumberFiled {
    constructor(port, label = 'Puerto', placeholder = '443') {
        super()
    ...
    validate() {
        if (!this.getValue()) {
            throw new Error(`Establecer un puerto válido para ${this._label.innerText || 'la configuración'}`)
        }
    }
}
...
export class ConfigIcon extends TextFiled {
    constructor(url) {
        ...
        this._upload.innerHTML = '<i class="fas fa-upload"></i>'
        ...
        this._view.innerHTML = '<i class="fas fa-eye"></i>'
